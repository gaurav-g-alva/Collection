...
def init_db():
    ...
    conn.close()
###
# ---------- saved timetables: schema + helpers ----------
# --- saved timetables schema + helpers (put near your other DB helpers) ---
import json, sqlite3
from typing import Tuple, Any, Dict, List, Optional

def create_saved_timetables_table():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS saved_timetables (
...
def _serialize_chromosome_for_save(chromosome, all_data) -> Dict[str, Any]:
    """Minimal, stable snapshot for rendering later."""
    payload = {
        "meta": {
        ...
        "scheduled": [],
        "lookups": {
            "semesters": {sid: s.semester_number for sid, s in all_data["semesters_by_id"].items()},
            "courses":   {cid: c.code            for cid, c in all_data["courses_by_id"].items()},
            "faculty":   {fid: f.name            for fid, f in all_data["faculty_by_id"].items()}
        }
    }
    for sc in getattr(chromosome, "scheduled_classes", []):
        payload["scheduled"].append({
            "semester_id": sc.semester_id,
            "course_id":   sc.course_id,
            "faculty_ids": list(sc.faculty_ids),
            "day":         sc.day,
            "start_period": sc.start_period,
            "end_period":   sc.end_period,
            "is_lab":       bool(sc.is_lab),
        })
    return payload

def add_saved_timetable(user_id: int, name: str, generated_on: str,
                        chromosome, all_data, overwrite: bool = False) -> Tuple[bool, str]:
    """
    Save a timetable snapshot for (user_id, name).
    If overwrite=True, update existing row; otherwise enforce uniqueness.
    """
    conn = get_db_connection()
    cur = conn.cursor()
    name = (name or "").strip()
    ...
    data_json = json.dumps(_serialize_chromosome_for_save(chromosome, all_data))

    try:
        if overwrite:
            cur.execute("""
                UPDATE saved_timetables
                   SET generated_on = ?, data_json = ?
                 WHERE user_id = ? AND timetable_name = ?
            """, (generated_on, data_json, user_id, name))
            if cur.rowcount == 0:
                cur.execute("""
                    INSERT INTO saved_timetables (user_id, timetable_name, generated_on, data_json)
                    VALUES (?, ?, ?, ?)
                """, (user_id, name, generated_on, data_json))
        else:
            cur.execute("""
                INSERT INTO saved_timetables (user_id, timetable_name, generated_on, data_json)
                VALUES (?, ?, ?, ?)
        ...
        return False, "A timetable with this name already exists. Enable overwrite to replace it."
    finally:
        conn.close()

def get_saved_timetables(user_id: int) -> List[sqlite3.Row]:
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""
        SELECT id, timetable_name, generated_on
          FROM saved_timetables
         WHERE user_id = ?
         ORDER BY datetime(generated_on) DESC
    """, (user_id,))
    rows = cur.fetchall()
    conn.close()
    return rows

def load_saved_timetable_data(user_id: int, saved_id: int) -> Optional[Dict[str, Any]]:
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""
        SELECT timetable_name, generated_on, data_json
          FROM saved_timetables
...
def delete_saved_timetable(user_id: int, saved_id: int) -> bool:
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("DELETE FROM saved_timetables WHERE user_id = ? AND id = ?", (user_id, saved_id))
    conn.commit()
    ok = cur.rowcount > 0
    conn.close()
    return ok
# put near your other saved-timetable helpers
def delete_saved_timetable_by_name(user_id: int, timetable_name: str) -> bool:
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""
        DELETE FROM saved_timetables
         WHERE user_id = ? AND timetable_name = ?
    """, (user_id, timetable_name))
    conn.commit()
    ok = cur.rowcount > 0
    conn.close()
    return ok
