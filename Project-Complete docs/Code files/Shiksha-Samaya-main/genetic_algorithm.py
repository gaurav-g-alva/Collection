...
def build_faculty_prefs_map(preferences_rows):
    """
    preferences_rows: iterable of rows/dicts having:
      faculty_id, day (e.g., 'Monday'), period_start, period_end, preference_type ('blocked'|'preferred')
    Returns:
      { fac_id: { 'Monday': {'blocked':[(s,e),...], 'preferred':[(s,e),...]}, ... }, ... }
    """
    out = defaultdict(lambda: defaultdict(lambda: {"blocked": [], "preferred": []}))
    for r in preferences_rows:
        fid = r["faculty_id"]
        day = r["day"]
        ps = int(r["period_start"])
        pe = int(r["period_end"])
        kind = r["preference_type"]  # 'blocked' or 'preferred'
        out[fid][day][kind].append((ps, pe))
    return out

###

class GeneticAlgorithm:
    def __init__(self, classes_to_schedule, all_data,
                 population_size=200, generations=1000,
                 mutation_rate=0.05, mutation_chance_smart=0.8,
                 crossover_rate=0.8,
                 faculty_prefs_map=None):
        """
        ...
        self.faculty_prefs_map = faculty_prefs_map or {}
    ...
    # --- helpers used by placement & fitness ---
    def _violates_blocked(self, sc) -> bool:
        """Return True if any faculty has a 'blocked' window overlapping sc."""
        prefs = self.faculty_prefs_map
        day = sc.day
        for fid in sc.faculty_ids:
            dayprefs = prefs.get(fid, {}).get(day, {})
            for (ps, pe) in dayprefs.get("blocked", []):
                if _periods_overlap(sc.start_period, sc.end_period, ps, pe):
                    return True
        return False

    def _soft_pref_penalty(self, sc, per_period=2.0) -> float:
        """Penalty if no preferred window overlaps the class window (when prefs exist that day)."""
        prefs = self.faculty_prefs_map
        day = sc.day
        total = 0.0
        for fid in sc.faculty_ids:
            dayprefs = prefs.get(fid, {}).get(day, {})
            prefers = dayprefs.get("preferred", [])
            if not prefers:
                continue
            matched = any(_periods_overlap(sc.start_period, sc.end_period, ps, pe) for (ps, pe) in prefers)
            if not matched:
                total += per_period * (sc.end_period - sc.start_period + 1)
        return total

    # Where you place/mutate classes, skip blocked candidates (example):
    def _try_place_without_blocked(self, sc, candidate_day, candidate_start):
        sc.day = candidate_day
        sc.start_period = candidate_start
        sc.end_period = candidate_start + sc.periods_count - 1
        return not self._violates_blocked(sc)

    # In your initial placement and mutation code, before accepting a day/slot:
    #   if not self._try_place_without_blocked(sc, day, start): continue

    # In your evaluate_fitness(), add:
    def evaluate_fitness(self, chromosome):
        fitness = 0.0
        HARD = 1000.0   # hard penalty per period for blocked
        SOFT = 2.0      # soft pref penalty per period

        # ... your current penalties & rewards ...

        for sc in chromosome.scheduled_classes:
            if self._violates_blocked(sc):
                fitness -= HARD * (sc.end_period - sc.start_period + 1)
            fitness -= self._soft_pref_penalty(sc, per_period=SOFT)
